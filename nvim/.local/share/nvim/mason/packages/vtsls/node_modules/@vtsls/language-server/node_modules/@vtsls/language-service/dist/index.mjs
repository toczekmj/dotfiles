    ...
    FileConfigurationManager = class extends Disposable {
      ...
      getPreferences(document) {
        ...
        const preferences = {
          ...
          autoImportFileExcludePatterns: this.getAutoImportFileExcludePatternsPreference(preferencesConfig, workspace.getWorkspaceFolder(document.uri)?.uri),
      ...
      getMaximumHoverLength(document) {
        ...
        const maximumHoverLength = workspace.getConfiguration("js/ts", document).get("hover.maximumLength", defaultMaxLength);
  ...
  "src/typescript-language-features/src/languageFeatures/codeLens/baseCodeLensProvider.ts"() {
    ...
    TypeScriptBaseCodeLensProvider = class extends Disposable {
      ...
      async provideCodeLenses(document, token) {
        ...
        response.body?.childItems?.forEach((item) => this.walkNavTree(document, item, void 0, referenceableSpans));
  ...
  "src/typescript-language-features/src/languageFeatures/copyPaste.ts"() {
    ...
    DocumentPasteProvider = class _DocumentPasteProvider {
      ...
      async prepareDocumentPaste(document, ranges, dataTransfer, token) {
        ...
        if (response) {
          ...
          dataTransfer.set(
            ...
            new DataTransferItem(new CopyMetadata(document.uri, ranges, void 0))
          );
        } else {
          dataTransfer.set(
            _DocumentPasteProvider.metadataMimeType,
            new DataTransferItem(new CopyMetadata(document.uri, ranges, copyRequest))
          );
        }
      }
      async provideDocumentPasteEdits(document, ranges, dataTransfer, _context, token) {
        ...
        const pasteCts = new CancellationTokenSource();
      ...
      getFixesForTsCodeAction(document, diagnostic, action) {
        ...
        if (copilot?.isActive) {
          ...
          if (action.fixName === classIncorrectlyImplementsInterface) {
            ...
            message = l10n.t("Implement the stubbed-out class members for {0} with a useful implementation.", document.getText(diagnostic.range));
            ...
            title = l10n.t(`Implement missing function declaration '{0}' using AI`, document.getText(diagnostic.range));
            message = l10n.t(`Provide a reasonable implementation of the function {0} given its type and the context it's called in.`, document.getText(diagnostic.range));
            expand = { kind: "code-action", action };
          ...
          const destinationItems = body.files.map((file3) => {
            ...
            if (workspaceFolder) {
              ...
              if (relativeQuery) {
                ...
                const relativePath = convertRelativePath(path7.relative(path7.dirname(document.uri.fsPath), uri.fsPath));
      ...
      dispatchEvent(event) {
        switch (event.event) {
          ...
          case "createDirectoryWatcher" /* createDirectoryWatcher */: {
            ...
            if (process.platform === "darwin" && fpath === path10.join(homedir2(), "Library")) {
              return;
            }
    ...
    TypeScriptWorkspaceSymbolProvider = class _TypeScriptWorkspaceSymbolProvider {
      ...
      async toOpenedFiledPath(document) {
        if (document.uri.scheme === git) {
          try {
            const path15 = Uri.file(JSON.parse(document.uri.query)?.path);
